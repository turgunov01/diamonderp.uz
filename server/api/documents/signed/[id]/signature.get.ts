...
export default eventHandler(async (event) => {
  ...
  if (!signaturePath) {
    throw createError({ statusCode: 404, message: 'Подпись для этого документа не найдена.' })
  }
  ...
  if (!signedUrl) {
    throw createError({ statusCode: 500, message: 'Не удалось сформировать ссылку на подпись.' })
  }
