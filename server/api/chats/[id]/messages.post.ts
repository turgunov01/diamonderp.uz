...
export default eventHandler(async (event) => {
  ...
  if (!body?.authorId || !body?.content) {
    throw createError({ statusCode: 400, message: 'Поля authorId и content обязательны.' })
  }
  ...
  if (!inserted?.id) {
    throw createError({ statusCode: 500, message: 'Postgres не вернул id созданного сообщения.' })
  }
